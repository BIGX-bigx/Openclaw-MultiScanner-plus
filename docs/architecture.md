...
ClawMatrix 的定位是一套独立的多阶段融合型 OpenClaw 信任边界扫描器，而不是单纯基于 Skill 扫描工具的附加模块。原 OpenClaw Skill Guard 可以作为第二层 Skill 生态审计引擎被吸收，但 ClawMatrix 的核心目标是覆盖 OpenClaw 的配置、状态目录、Gateway、Control UI、browser-control、内部方法族、Skill、记忆与任务等多个关键能力面。
...
ClawMatrix 提供轻量能力识别，同时在 `engines/skill-guard` 内置 Rust 版 Skill Guard 深度审计引擎。扫描器可以在存在 Rust/cargo 的环境中自动调用该引擎，并把运行状态和深度报告作为第二层证据的一部分。这样第二层不是简单重写已有能力，而是以“轻量扫描 + 深度引擎”的方式保留并融合原工具能力。
...
ClawMatrix 输出 JSON、Markdown 和中文 HTML。JSON 用于后续自动化对比，Markdown 用于组会汇报，HTML 用于可视化展示。每个发现项包含层级、风险编号、严重度、标题、证据和解释。建议后续继续增加 SARIF 输出，并把 Caramel-Pudding、Skill Guard、SkillDance 的结果统一归并为同一 evidence schema。
